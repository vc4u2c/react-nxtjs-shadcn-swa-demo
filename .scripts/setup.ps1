Param(
  ...
  [String]$NistNvdApiBaseUrl,
  [String]$NistNvdApiKeySecret
)
...
if (-not $ProjectShortName) {
  if ($GitHubRepositoryName) {
    $ProjectShortName = $GitHubRepositoryName
  }

  if (-not $ProjectShortName) { $MissingParameterValues = $true }
}

if (-not $NistNvdApiBaseUrl) {
  $MissingParameterValues = $true
}

if (-not $NistNvdApiKeySecret) {
  $MissingParameterValues = $true
}
...
$ParametersTableData = @{
  ...
  "NistNvdApiBaseUrl"               = $NistNvdApiBaseUrl
  "NistNvdApiKeySecret"             = $NistNvdApiKeySecret
}
...
function SetVariables() {
  ...
  gh variable set PROJECT_SHORT_NAME --body $ProjectShortName --repo $repoUrl
}
...
function SetEnvironmentVariablesAndSecrets {
  ...
  gh variable set AZURE_RESOURCE_GROUP_NAME --body "rg-$ProjectName-$environmentAbbr" --env $environmentName --repo $repoUrl
  ...
  gh variable set NIST_NVD_BASE_URL --body "$NistNvdApiBaseUrl" --env $environmentName --repo $repoUrl
  gh secret set NIST_NVD_API_KEY_SECRET --body "$NistNvdApiKeySecret" --env $environmentName --repo $repoUrl
}
